        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Update Skill Sectiion</li>
            </ol>
          </div> 
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <section class="col-md-8 offset-md-2">
           ...
           <div class="card">
              <div class="card-header" style="background-color: #086A87;color: white">
                <h5>Update Skill Sectiion
                  <a  href="{{route('sections.skill-view')}}" class="btn btn-warning btn-sm float-right"><i class="fa fa-list"> Skill Section List</i></a>
                </h5>
              </div> 
            <div class="card-body">
                
              <form method="post" action="{{route('sections.skill-update',$editdata->id)}}" id="myform"enctype="multipart/form-data">
                @csrf
                <div class="form-row">

                  <div class="form-group col-md-12">
                    <label for="title">Skill Title</label>
                    <textarea type="text" rows="4" name="title" id="title" class="form-control" placeholder="Enter Skill Title">{{$editdata->title}}</textarea>
                    <font style="color:red">{{($errors)->has('title')?($errors->first('title')):''}}</font>
                  </div>

                  <div class="form-group col-md-12">
                    <label for="description">Skill Description</label>
                    <textarea type="text" name="description" id="description" class="form-control" placeholder="Enter Skill Description" rows="6">{{$editdata->description}}</textarea>
                ...
                <input type="submit" value=" Update Skill Section" name="submit" class="btn btn-primary btn-block float-right" style="font-weight: bold;font-size: 18px">
                  </div>
                </div> 
              </form>
